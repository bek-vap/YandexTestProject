...
> **Про демо-сервер.** Прототип развёрнут на VPS 1 CPU / 2 GB. Чтобы headless-браузер
> стабильно работал в таком объёме памяти, на демо задан лимит `YANDEX_REVIEW_LIMIT=100`
> (отзывы кэшируются в БД и показываются по 50 на страницу). Средний рейтинг и счётчики
> (число оценок и отзывов) — **реальные**, читаются напрямую с Яндекса, поэтому счётчик
> показывает полное число (напр. 2719), а список — то, что успели спарсить. Сам парсер
> поддерживает полный сбор ~600 отзывов; на машине с большим объёмом памяти лимит убирается
> (`YANDEX_REVIEW_LIMIT=` пусто = тянуть все).
